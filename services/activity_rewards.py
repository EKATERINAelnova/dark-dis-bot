...
async def grant_activity_reward(
    ...
    if reward_kind not in REWARD_KINDS:
        raise ValueError(
            f"Неизвестный тип награды: {reward_kind}"
        )
    ...
    async with get_db() as db:
        try:
            ...
            activity_status = str(activity_row[0])
            activity_type = str(activity_row[1])
